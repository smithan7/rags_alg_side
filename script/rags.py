...
class RAGS:
	...
	def waiting_on_RAGS_callback( self, query ):
			...
			if query.request_type == 0:
				print "recieved RAGS scan goals request"
				# requesting edges to scan, send empty list
				sg = Scan_Goals()
				for i in range(0,3):
					sg.lats.append( query.my_lat + (random.random()-0.5) * 0.01 )
					sg.lons.append( query.my_lon + (random.random()-0.5) * 0.01 )
					sg.indices.append( i )
				print "sending new scan vertices: ", sg.indices
